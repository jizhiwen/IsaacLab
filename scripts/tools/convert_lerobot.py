...
DATASET_PATH = "/mnt/data/datasets/generated_dataset_05291446.hdf5"
IMAGE_PATH = "/mnt/data/datasets/_isaaclab_out_/images-2025-05-29_14-46-43"
OUTPUT_DATASET_DIR = "/mnt/data/home/robot2/datasets/lerobot/synthetic-realman-eye-to-hand-demo"
VIDEO_KEY = "ego_view"
...
DATASET_NAME = "RM65" # Used in info.json
...
def main():
    ...
    with h5py.File(DATASET_PATH, 'r') as f:
        ...
        all_episode_details = []
        task_description = "Pick up the red square and place it on top of the blue square, and pick up the green square and place it on top of the red square."
        unique_tasks = {task_description: 1}
        global_fps = 24
        global_video_info = {"width": 256, "height": 256, "channels": 3}

        for dataset in range(0,len(f['data'])):
            episode_data = []
            states = np.array(f[f'data/demo_{dataset}/states/articulation/robot/joint_position'][:], np.float32)
            states[:,-2:] = [[0, 0] if finger[0] == 0.0 or finger[1] == 0.0  else [1000,1000] for finger in states[:,-2:]]
            ...
            episode_frames_pil = []
            for frame_idx in range(video_start, video_start + video_length):
            ...
            # for frame_idx in range(video_start, video_start + video_length):
            #     # normals video
            #     file_path_normal = os.path.join(IMAGE_PATH, normals_name_pattern.format(trial_num=episode_index, frame_idx=frame_idx))
            #     normals_pil = Image.open(file_path_normal)
            #     episode_normals_pil.append(normals_pil)
            # dataset_formatter.write_extra_video(episode_normals_pil, global_fps, episode_index, "normals")
            # episode_normals_pil.clear()

            # episode_segmentation_pil = []
            # for frame_idx in range(video_start, video_start + video_length):
            #     # segmentation video
            #     file_path_segmentation = os.path.join(IMAGE_PATH, segmentation_name_pattern.format(trial_num=episode_index, frame_idx=frame_idx))
            #     segmentation_pil = Image.open(file_path_segmentation)
            #     episode_segmentation_pil.append(segmentation_pil)
            # dataset_formatter.write_extra_video(episode_segmentation_pil, global_fps, episode_index, "semantic_segmentation")
            # episode_segmentation_pil.clear()
            
            # --- Generate shaded segmentation video
            video_filedir = dataset_formatter.get_extra_video_dir("shaded_segmentation")
            video_filename = f"episode_{episode_index:06d}.mp4"
            video_filepath = os.path.join(video_filedir, video_filename)
            encode_video(IMAGE_PATH, video_start, video_length,
                         "table_high_cam", video_filepath, 0, episode_index)
